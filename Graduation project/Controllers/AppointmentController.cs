...
namespace Graduation_project.Controllers
{
    ...
    public class AppointmentController : ControllerBase
    {
        ...
        [HttpGet("TodaysAppointmentsForDoctor")]
        [Authorize (Roles ="Doctor")]
        public async Task<IActionResult> GetTodaysAppointmentsForDoctor([FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 10)
        {
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            if (string.IsNullOrEmpty(userId)) return Unauthorized();
            try
            {
                var response = await _appointmentService.GetTodaysAppointmentForDoctorAsync(userId, pageNumber, pageSize);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet("UpcomingAppointmentsForDoctor")]
        [Authorize(Roles = "Doctor")]
        public async Task<IActionResult> GetUpcomingAppointmentsForDoctor([FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 10)
        {
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            if (string.IsNullOrEmpty(userId)) return Unauthorized();
            try
            {
                var response = await _appointmentService.GetUpcomingAppointmentsForDoctorAsync(userId, pageNumber, pageSize);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet("PatientsForDoctor")]
        [Authorize(Roles = "Doctor")]
        public async Task<IActionResult> GetPatientsForDoctor([FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 10)
        {
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            if (string.IsNullOrEmpty(userId)) return Unauthorized();
            try
            {
                var response = await _appointmentService.GetPatientsForDoctorAsync(userId, pageNumber, pageSize);
                return Ok(response);
            }
            catch (Exception ex)
            {
